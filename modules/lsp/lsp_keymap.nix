...
vim.api.nvim_create_autocmd("LspAttach", {
	...
	callback = function(ev)
      ...
      local shared_opts = { buffer = ev.buf }
		...
		local_map("n", "<leader>cf", function() vim.lsp.buf.format { timeout_ms = 3000 }  end, { desc = "Format Document" })
		local_map("v", "<leader>cf", function() vim.lsp.buf.format { timeout_ms = 3000 } end, { desc = "Format Range" })
